...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        return self.linear(x)  # (batch_size, input_size) -> (batch_size, 1)


# 生成一个样本, 样本的生成方法，代表了我们要学习的规律
# 随机生成一个5维向量，取5个里面的最大值下标作为样本分类编号
def build_sample():
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            predict = torch.argmax(y_p)
            correct += (predict == y_t).sum().item()
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        ...
        for i in range(result.size(0)):
            res_max.append(torch.argmax(result[i]))
        # print(res_max)
    for vec, res in zip(input_vec, res_max):
